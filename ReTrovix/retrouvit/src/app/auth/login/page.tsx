...
export default function LoginPage() {
  ...
  /**
   * Connexion Google (CDC §5.1/§6.1) : récupère l'URL de consentement
   * construite par le backend (client_id configuré côté serveur) et
   * redirige l'utilisateur. Le callback /auth/google/callback échange
   * le code contre les JWT internes.
   */
  const handleGoogleLogin = async () => {
    setError("");
    try {
      const { googleAuthApi } = await import("@/lib/api-google");
      const redirectUri = `${window.location.origin}/auth/google/callback`;
      const { url } = await googleAuthApi.getAuthorizationUrl(redirectUri);
      ...
      <div className="flex flex-1 flex-col justify-center px-4 py-12 sm:px-6 lg:px-12 bg-background">
        <div className="mx-auto w-full max-w-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Button variant="outline" className="h-11 rounded-xl font-medium" disabled title="Bientôt disponible">
