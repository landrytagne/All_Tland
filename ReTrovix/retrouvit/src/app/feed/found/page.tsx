...
import { foundObjectsApi, type FoundObjectResponse, type PageResponse } from "@/lib/api-objects";
import { categories, cities } from "@/lib/data";
import { useTranslation } from "@/lib/i18n";

const PAGE_SIZE = 12;

export default function FoundObjectsFeedPage() {
  return (
    <AuthGuard>
      <FoundObjectsFeed />
    </AuthGuard>
...
function FoundObjectsFeed() {
  const { t } = useTranslation();
  const [pageData, setPageData] = React.useState<PageResponse<FoundObjectResponse> | null>(null);
    ...
    const fetchObjects = async () => {
      ...
      try {
        if (selectedCategory !== "all") {
          const data = await foundObjectsApi.getByCategory(selectedCategory, currentPage, PAGE_SIZE);
          setPageData(data);
        } else if (selectedCity !== "all") {
          const data = await foundObjectsApi.getByCity(selectedCity, currentPage, PAGE_SIZE);
          setPageData(data);
        } else {
          const data = await foundObjectsApi.getAll(currentPage, PAGE_SIZE);
          setPageData(data);
        }
      } catch (err) {
        console.warn("Could not fetch found objects:", err);
      } finally {
        setIsLoading(false);
      }
    };
        ...
        <div className="mb-6 animate-fade-in">
          <div className="flex items-center gap-3 mb-1">
            <div className="h-10 w-10 rounded-xl bg-forest/10 dark:bg-forest-light/10 flex items-center justify-center">
              <Search className="h-5 w-5 text-forest dark:text-forest-light" />
            </div>
            <div>
              <h1 className="text-2xl font-bold">{t("feed.foundObjects")}</h1>
              <p className="text-sm text-muted-foreground">
                {t("feed.foundObjects") === "Objets trouvés" ? "Un objet trouvé ? Vérifiez s'il correspond à une annonce." : "Found an object? Check if it matches a listing."}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(selectedCategory !== "all" || selectedCity !== "all" || searchQuery) && (
              <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-1">
                <X className="h-3 w-3" /> {t("landing.clearFilters")}
              </Button>
            )}
          </div>
        </div>
